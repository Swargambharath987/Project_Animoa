...
def show_logout_feedback_form(translations):
    """Show a streamlined logout feedback form"""
    logout_container = st.container()
    
    with logout_container:
        st.subheader(translations.get("feedback_before_leave", "Before you go..."))
        
        # Simple 5-star rating with emoji
        cols = st.columns(5)
        rating = 0
        
        # Initialize session state for rating if it doesn't exist
        ...
        for i in range(5):
            with cols[i]:
                # Use filled star if rating is high enough
                star = "⭐" if i < st.session_state.logout_rating else "☆"
                if st.button(star, key=f"star_{i+1}"):
                    st.session_state.logout_rating = i + 1
                    st.rerun()
        
        # Only show the comment field if a rating was selected
        if st.session_state.logout_rating > 0:
            # Optional comment
            feedback = st.text_area(
                translations.get("quick_thoughts", "Any quick thoughts? (optional)"),
                max_chars=200,
                placeholder=translations.get("feedback_placeholder", "What did you like or what could we improve?")
            )
            
            # Action buttons
            col1, col2 = st.columns(2)
            with col1:
                if st.button(translations.get("submit_and_logout", "Submit & Logout"), type="primary"):
                    try:
                        # Save feedback
                        feedback_data = {
                        ...
                        # If saving fails, still proceed with logout
                        pass
                    return True  # Signal to proceed with logout
            
            with col2:
                if st.button(translations.get("just_logout", "Just Logout")):
                    return True  # Skip feedback but proceed with logout
        else:
            # If no rating selected, just show the logout options
            col1, col2 = st.columns(2)
            with col1:
                if st.button(translations.get("rate_and_logout", "Rate & Logout"), type="primary"):
                    st.info(translations.get("please_select_rating", "Please select a rating first"))
                    st.rerun()
    ...
    return False  # By default, don't proceed with logout
...
def main():
    ...
    if st.session_state.show_logout_feedback:
        should_logout = show_logout_feedback_form(translations)
